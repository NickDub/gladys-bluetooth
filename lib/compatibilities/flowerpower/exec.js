									...
									function(callback) {
										...
										if (typeToUpdate[type]) {
											...
											flowerPower.readSunlight(function (error, value) {
												utils.sendDeviceState('flowerpower', type, devicetypeId[type], utils.photonsToLux(value));
									...
									function(callback) {
										...
										if (typeToUpdate[type]) {
											...
											flowerPower.readSoilTemperature(function (error, value) {
												utils.sendDeviceState('flowerpower', type, devicetypeId[type], value.toFixed(2));
												typeToUpdate[type] = false;
												callback();
											});
										} else callback();
									...
									function(callback) {
										...
										if (typeToUpdate[type]) {
											...
											flowerPower.readSoilElectricalConductivity(function (error, value) {
												utils.sendDeviceState('flowerpower', type, devicetypeId[type], (value / 1000).toFixed(2));
												typeToUpdate[type] = false;
												callback();
											});
										} else callback();
